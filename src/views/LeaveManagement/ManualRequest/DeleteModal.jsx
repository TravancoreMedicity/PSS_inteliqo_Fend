...
const DeleteModal = ({ open, setOpen, punchMastdata, setCount }) => {
    ...
    const submitRequest = useCallback(async () => {
        ...
        if (reason === '') {
            ...
            let result = await axioslogin.post("/manualRequest/deletePunchMasterSingleRow", postData);
            const { success } = result.data;
            if (success === 1) {

                let result = await axioslogin.patch("/manualRequest/inactiveStatus", deleteId);
                const { success } = result.data;
                if (success === 1) {
                    ...
                    errorNofity("Error While Inactiving Data ! Contact IT")
                }
            } else {
                errorNofity('Error Punch Master Data Updarion ! Contact IT')
                setOpen(false)
            }
