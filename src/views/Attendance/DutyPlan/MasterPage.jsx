...
const MasterPage = () => {
  ...
  const departmentShiftt = useSelector(
    (state) => state.getDepartmentShiftData.deptShiftData,
    _.isEqual,
  )
  ...
  const onClickDutyPlanButton = useCallback(
    async (e) => {
      setOpen(true)
      e.preventDefault()
      ...
      if (deptName === 0 || deptSecName === 0) {
        ...
        const checkPunchMarkingHr = await axioslogin.post(
        ...
        if (success === 0 || success === 1) {
          const lastUpdateDate =
            data?.length === 0
              ? moment(startOfMonth(new Date(fromDate))).format('YYYY-MM-DD')
              : moment(new Date(data[0]?.last_update_date)).format('YYYY-MM-DD')
          ...
          if (lastUpdateDate === lastDay_month) {
            ...
            getEmployeeDetlDutyPlanBased(postData).then((emplyDataArray) => {
              ...
              if (status === 1) {
                ...
                //process function
                dutyPlanInsertFun(planState, commonSettings, holidayList, data, deptShift).then(
                  (values) => {
  ...
  return (
    ...
    >
      ...
      <Box
        sx={{ display: 'flex', flex: { xs: 4, sm: 4, md: 4, lg: 4, xl: 3 }, flexDirection: 'row' }}
      >
          ...
          <Box sx={{ p: 0.2 }}>
            <Tooltip title="Save" followCursor placement="top" arrow>
              <Button
                ...
                variant="outlined"
                color="neutral"
                onClick={onClickSaveShiftUpdation}
                sx={{
                  color: '#81c784',
                }}
              >
