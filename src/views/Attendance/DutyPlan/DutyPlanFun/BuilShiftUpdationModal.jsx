...
const BuilShiftUpdationModal = ({ open, handleChange, emNo, updation }) => {
  ...
  const { notapplicable_shift, default_shift, week_off_day, week_off_count } = commonSettings
  ...
  const multiShiftSubmit = async () => {
    ...
    const noOfWOff = newPlanUpdate?.filter((k) => k?.shift_id === week_off_day)
        ...
        <DialogContent sx={{ display: 'flex', minWidth: 500, flexDirection: 'column' }}>
          <Box sx={{ display: 'flex', flex: 1 }}>
            <FormControl fullWidth>
              <Select value={shift} size="small" onChange={(e) => setShift(e.target.value)}>
                ...
                {deptShift &&
                  deptShift.map((val, index) => (
                    <MenuItem
                      ...
                      key={index}
                      disabled={
                        val.shiftcode === notapplicable_shift || val.shiftcode === default_shift
                          ? true
                          : false
                      }
                    >
