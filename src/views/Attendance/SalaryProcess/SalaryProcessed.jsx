...
const onClickProcess = useCallback(async () => {
  ...
  try {
    // Check if selected date value is valid
    if (!isValid(value) || value === null) {
      warningNofity('Invalid or empty date selected!')
      return
    }

    // Format the start and end of the selected month as 'yyyy-MM-dd'
    const fromDate = format(startOfMonth(new Date(value)), 'yyyy-MM-dd')
    const toDate = format(endOfMonth(new Date(value)), 'yyyy-MM-dd')
    ...
    // Prepare department and section data based on whether "all" option is selected
    const getEmpData =
      all === true
        ? {
            em_department: allDept?.map((val) => val?.dept_id),
    ...
    // Set API endpoint based on whether "all" employees are selected or department-wise
    const empEndpoint = all
      ? '/payrollprocess/getAllEmployee'
      : '/payrollprocess/getEmpNoDeptWise'
    ...
    // Extract employee IDs from the employee data
    const empIds = employeeData?.map((val) => val?.em_id)

    // Prepare punch data request payload
    const postdata = { emp_id: empIds, from: fromDate, to: toDate }

    // Fetch punch details for each employee
    const result2 = await axioslogin.post('/payrollprocess/punchbiId', postdata)
    const { success, data } = result2.data
    ...
    // Store final computed array into state
    setArray(finalDataArry)
  ...
  const downloadFormat = useCallback(async () => {
    ...
    const fromDate = format(startOfMonth(new Date(value)), 'yyyy-MM-dd')
    const toDate = format(endOfMonth(new Date(value)), 'yyyy-MM-dd')
    const dateRange = eachDayOfInterval({ start: new Date(fromDate), end: new Date(toDate) }).map(
    ...
    const exportToExcel = async (formattedData) => {
      ...
      const days = [
        ...
        '',
        '',
        ...values?.map((val) => (val?.holiday === 1 ? val?.holidayDays?.toLowerCase() : val?.days)),
      ]
      ...
      if (succes !== 1 || !isValidDate) {
        warningNofity(
          all
            ...
            : 'No Employee Under this Department || Department Section',
        )
        return
      }
