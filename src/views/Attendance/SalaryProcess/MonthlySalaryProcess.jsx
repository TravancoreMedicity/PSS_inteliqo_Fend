...
} from 'src/views/CommonCode/CommonReactQueries'
import { useQuery } from 'react-query'
import { useSelector } from 'react-redux'
import { endOfMonth, format, startOfMonth } from 'date-fns'
import { axioslogin } from 'src/views/Axios/Axios'
import { errorNofity, succesNofity, warningNofity } from 'src/views/CommonCode/Commonfunc'
import { attendnaceCountCalculationFunc, getAllPunchmastData } from './SalaryProcessFunctions'
...
const MonthlySalaryProcess = () => {
  ...
  const onProcessClick = useCallback(async () => {
    ...
    if (success === 1) {
      ...
      const getPunchMarkingHr_table = await axioslogin.post(
        '/payrollprocess/getmonthdeptlist/',
        getPunchMarkTablePostData,
      )
      ...
      if (succ === 1) {
        ...
        const findDept = [...new Set(deptSectionData?.map((e) => e.dept_id))]?.map((dept) => {
          return {
            ...
            section: deptSectionData
              ?.filter((val) => val.dept_id === dept)
              .map((v) => {
                return {
                  ...v,
                  updated:
                    punchMarkingTableData?.find((e) => v.sect_id === e.sect_id)?.last_update_date ??
                    format(startOfMonth(new Date(value)), 'yyyy-MM-dd'),
                  status:
                    punchMarkingTableData?.find((e) => v.sect_id === e.sect_id)?.salary_status ?? 0,
                }
              }),
          }
        })
        ...
        if (succes === 1 && value !== null) {
          ...
          getAllPunchmastData(postdata).then((punchmastdata) => {
            ...
            if (status === 1) {
              attendnaceCountCalculationFunc(employeeData, data, value, commonSettings).then(
                async (allData) => {
                  ...
                  if (status === 1 && data?.length !== 0) {
                    ...
                    const result1 = await axioslogin.post(
                      '/payrollprocess/create/processedSalary',
                      data,
                    )
                    ...
                    if (success === 1) {
                      ...
                      const insertPunchMarkTable = await axioslogin.post(
                        '/payrollprocess/insert/monthlyprocess',
                        postData_PunchMarkHR,
                      )
                      const { success } = insertPunchMarkTable.data
                      if (success === 1) {
                        const findDept = [...new Set(deptSectionData?.map((e) => e.dept_id))]?.map(
                          (dept) => {
                            return {
                              dept_id: dept,
                              dept_name: deptSectionData?.find((e) => e.dept_id === dept)
                                ?.dept_name,
                              section: deptSectionData
                                ?.filter((val) => val.dept_id === dept)
                                .map((v) => {
                                  return {
                                    ...v,
                                    updated: format(startOfMonth(new Date(value)), 'yyyy-MM-dd'),
                                    status: 1,
                                  }
                                }),
                            }
                    ...
                    warningNofity('Error While Attendance Calculation')
                    setOpenBkDrop(false)
                  }
                },
              )
            } else {
              warningNofity('Error While Geting All Employee PunchMast Data')
              setOpenBkDrop(false)
            }
          })
        } else {
  ...
  const deleteAttendanceMarkingProcess = useCallback(
    async (dept, section, date) => {
      const getPunchMarkTablePostData = {
                  ...
                  section: deptSectionData
                    ...
                    .map((v) => {
                      return {
                        ...v,
                        updated:
                          punchMarkingTableData?.find((e) => v.sect_id === e.sect_id)
                            ?.last_update_date ??
                          format(startOfMonth(new Date(value)), 'yyyy-MM-dd'),
                        status:
                          punchMarkingTableData?.find((e) => v.sect_id === e.sect_id)
  ...
  const insertSectionAttendance = useCallback(
    async (dept, section, date) => {
      setOpenBkDrop(true)
            ...
            attendnaceCountCalculationFunc(employeeData, data, value, commonSettings).then(
              async (allData) => {
                ...
                if (status === 1 && data?.length !== 0) {
                  ...
                  if (success === 1) {
                    const result1 = await axioslogin.post(
                      '/payrollprocess/activate/processedSalary',
                      postDta,
                    )
                    const { success, message } = result1.data
                    if (success === 1) {
                      const result = await axioslogin.get(
                        '/payrollprocess/getAcriveDepartmentSection/',
                      )
                      const { success, data } = result.data
                      const deptSectionData = data
                      if (success === 1) {
                        ...
                        if (succ === 1) {
                          const punchMarkingTableData = data
                          const findDept = [
                            ...new Set(deptSectionData?.map((e) => e.dept_id)),
                          ]?.map((dept) => {
                            return {
                              dept_id: dept,
                              dept_name: deptSectionData?.find((e) => e.dept_id === dept)
  ...
  return (
    <CustomLayout title="Monthly Salary Process" displayClose={true}>
      ...
      <CustomBackDrop
        open={openBkDrop}
        text="!!! Please wait...Monthly Payroll Processing.... Do not Refesh or Reload the Browser !!!"
      />
      <Box sx={{ width: '100%' }}>
        ...
        <Box padding={1}>
          <Sheet sx={{ height: window.innerHeight - 180, overflow: 'auto' }}>
            <Table variant="soft" size="sm" hoverRow stickyHeader>
              <thead>
                <tr>
                  <th>#</th>
                        ...
                        <td>
                          {monthStart === e.updated ? (
                            <Chip
                              ...
                              size="sm"
                              variant="solid"
                              startDecorator={<CalendarMonthIcon fontSize="inherit" />}
                            >
                              {e.updated}
                            </Chip>
                          ...
                          <Box sx={{ display: 'flex', justifyContent: 'space-evenly' }}>
                            {e.status === 1 ? (
                              ...
                              <Chip
                                color="success"
                                onClick={(c) =>
                                  insertSectionAttendance(row.dept_id, e.sect_id, e.updated)
                                }
                                size="sm"
                                variant="outlined"
                              >
                            ...
                            {e.status === 1 ? (
                              <Chip
                                color="danger"
                                onClick={() =>
                                  deleteAttendanceMarkingProcess(row.dept_id, e.sect_id, e.updated)
                                }
                                size="sm"
                                variant="outlined"
                              >
